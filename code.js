...
${answerStr}
の順番・内容になるようにしてください。
具体的には、1問目の正解は「${ans[0]}」${problemNum >= 2 ? `、2問目の正解は「${ans[1]}」` : ''}${problemNum >= 3 ? `、3問目の正解は「${ans[2]}」` : ''}${problemNum >= 4 ? `、4問目の正解は「${ans[3]}」` : ''}${problemNum >= 5 ? `、5問目の正解は「${ans[4]}」` : ''}となるようにしてください。必ずこの指定通りに正解を割り当ててください。
...
---`;
  ...
  // --- で分割
  let split = geminiText.split(/^-{3,}$/m);
  // 最初の要素が空や「---」だけの場合は除去
  if (split[0].trim() === '' || split[0].trim() === '---') {
    split = split.slice(1);
  }
  ...
  // ans[]を2～6列目に、G列（7列目）にpartLabelを格納
  let ansCols = [];
  for (let i = 0; i < 5; i++) {
    ansCols.push(ans[i] || ''); // ansが足りない場合は空欄
  }
  answersSheet.appendRow([new Date(), ...ansCols, partLabel]);
...
function onFormSubmit(e) {
  ...
  // Geminiへのプロンプト作成
  let feedbackPrompt = `以下はTOEIC ${partLabel}形式の問題文と設問、受験者の実際の回答と正解です。
...
for (let i = 0; i < userAnswers.length; i++) {
  feedbackPrompt += `Q${i+1}: 回答=${userAnswers[i]} / 正解=${correctAnswers[i]}\n`;
}
feedbackPrompt += `
上記の「受験者の回答」と「正解」を比較し、各設問ごとに
・正しい答えとその根拠（本文から導ける理由）
・本文の和訳（対訳形式で）
・難易度の高い語彙とその日本語解説
を日本語でまとめてください。
長文問題のような、文書がついている問題の場合は、文書の和訳も含めてください。(「==============」は文書を区切るためのものです)
【重要】
・必ず「受験者の回答」と「正解」を答え合わせし、どこが正しく、どこが間違っているかを明確に指摘してください。
・単なる解説ではなく、「受験者の回答」を分析した個別のアドバイスになるよう注意してください。
・出力は必ずHTML形式（h2, strong, ul, li, p など）で返してください。マークダウンは使わないでください。
・適切に改行を行い、解説が見やすいようにしてください。
・解説内で長文の文書を表示する場合は、必ず枠（<div style="border:1px solid #ccc; padding:1em; margin:1em 0;">...</div> など）で囲んでください。`;
